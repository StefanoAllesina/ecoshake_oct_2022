...
The Authors grew different combinations of plants (chosen from a pool of twelve species) in field mesocosm. They subjected half of the communities to a drought treatment, and the other half were used as control. We are going to examine the biomass for each species/mesocosm at the end of the experiment for the plant grew in Belgium.
...
Similarly, we can count how many species are present at the end of the experiment for each mesocosm. To sort the table from the most speciose to the least, we use `arrange`
...
Now we would like to label each plot by its final composition, so that we can contrast communities subject to the drought treatment with those in the control group. In particular, we are going to add a community label to each row, obtained by stringing together all the `shortname`s of the species in each plot.

To accomplish this we need to group the data by `label` (i.e., by mescosm) and by `treatment` (i.e, drought/control); when we invoke `group_by`, we are adding an invisible column specifying the group structure; we can then perform operations on each group separately. This is one of the most useful features of `tidyverse`. 
...
- the data for each species is plotted in a separate panel
